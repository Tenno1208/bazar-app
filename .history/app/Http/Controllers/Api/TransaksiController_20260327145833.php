...
class TransaksiController extends Controller
{
    public function index()
    {
        ...
        // Ini agar kotak "Via QRIS" di dashboard ada isinya
        $countQris = Transaksi::where('metode', 'qris')->count();
        $countCash = Transaksi::where('metode', 'cash')->count();
        // -------------------------------

        // Tambahkan countQris dan countCash ke dalam compact
        return view('dashboard', compact('data', 'total', 'countQris', 'countCash'));
    }

    public function store(Request $request)
    {
        // Validasi simpel agar harga tidak kosong
        $request->validate([
