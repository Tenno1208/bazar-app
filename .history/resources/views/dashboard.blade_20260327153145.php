...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        body { font-family: 'Plus Jakarta Sans', sans-serif; background: var(--bg); margin: 0; color: var(--text); user-select: none; }

        /* Navbar */
        nav { background: white; padding: 15px 20px; display: flex; justify-content: space-around; align-items: center; box-shadow: 0 2px 15px rgba(0,0,0,0.05); position: sticky; top: 0; z-index: 1000; border-bottom: 1px solid #f1f5f9; }
        nav a { text-decoration: none; text-align: center; }
        nav .nav-label { font-size: 10px; font-weight: 800; display: block; margin-top: 4px; color: var(--text); }

        /* Toast Alert - Diperbaiki Animasinya */
        .toast { 
            position: fixed; 
            top: 20px; 
            left: 50%; 
        ...
        /* Splash */
        #splash { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(135deg, var(--primary), var(--secondary)); display: flex; flex-direction: column; align-items: center; justify-content: center; z-index: 10000; color: white; transition: 0.8s; }
        .bounce { animation: bounce 2s infinite; font-size: 80px; }
        @keyframes bounce { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-20px); } }

        .container { max-width: 500px; margin: auto; padding: 20px; }
        .header { background: linear-gradient(135deg, #6366f1, #4361ee); margin: -20px -20px 25px -20px; padding: 40px 25px; border-radius: 0 0 30px 30px; color: white; box-shadow: 0 10px 20px rgba(67, 97, 238, 0.2); }
        ...
        input, select { width: 100%; padding: 14px; margin-top: 8px; border-radius: 12px; border: 1.5px solid #e2e8f0; background: #f1f5f9; font-size: 16px; font-family: inherit; }
        
        .btn-submit { background: var(--primary); color: white; border: none; padding: 16px; width: 100%; border-radius: 15px; font-weight: 700; margin-top: 20px; cursor: pointer; box-shadow: 0 4px 12px rgba(67, 97, 238, 0.3); transition: 0.3s; }
        ...
        .btn-del { background: #fff1f2; color: #e11d48; border: none; padding: 8px 12px; border-radius: 10px; margin-left: 10px; font-weight: bold; cursor: pointer; }
        
        .modal { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.85); backdrop-filter: blur(5px); display: flex; align-items: center; justify-content: center; z-index: 9999; }
...
<body>
    ...
    @if(session('success'))
    <div class="toast animate-toast" style="border-left-color: var(--success);">
        <span>✅</span> {{ session('success') }}
    </div>
    @endif

    @if(session('error') || $stok <= 0)
    <div class="toast animate-toast" style="border-left-color: var(--danger);">
        <span>⚠️</span> {{ session('error') ?? 'Stok Habis!' }}
    </div>
    @endif

    <nav>
        <a href="/">🏠 <span class="nav-label" style="color: var(--primary);">KASIR</span></a>
        <a href="/stok" style="opacity: 0.5;">📦 <span class="nav-label">STOK</span></a>
        <a href="/laporan-keuntungan" style="opacity: 0.5;">📊 <span class="nav-label">LABA</span></a>
    </nav>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // 2. Logic Auto-Hide Toast (Diperbaiki)
            const toasts = document.querySelectorAll('.toast');
            toasts.forEach((toast) => {
                // Munculkan toast (sudah diatur via CSS animation slideIn)
                
                // Set timer untuk menghapus toast
                setTimeout(() => {
                    toast.style.top = '-100px'; // Geser ke atas
                    toast.style.opacity = '0';   // Hilangkan pelan-pelan
                    
                    // Benar-benar hapus dari DOM setelah animasi transisi selesai (0.5s)
                    setTimeout(() => {
                        toast.remove();
                    }, 500);
